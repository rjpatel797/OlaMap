...
function Gauge( canvas, options ) {
	...
	this.colors = {
		...
		// START - Deprecated
		redBand: options.colorOfRedBand || options.redColor || 'rgba(255, 0, 0, 0.6)',
		yelBand: options.colorOfYellowBand || options.yellowColor || 'rgba(255, 215, 0, 0.4)',
		grnBand: options.colorOfGreenBand || options.greenColor || 'rgba(0, 255, 0, 0.4)'
		// END - Deprecated
	};
	...
	this.drawCaption = function( label ) {
		if ( label ) {
			var fontSize = this.radius / 3.5;
			styleText( this.c2d, fontSize.toFixed(0) + 'px monospace');
	...
	this.drawValues = function( min, max, value, decimals ) {
		...
		styleText( this.c2d, fontSize.toFixed(0) + 'px');
